...
**[How it works](#how-it-works)** · **[Journal](#journal)** · **[Search](#search)** · **[Data and privacy](#data-and-privacy)**
...
If recording or note generation fails, Pi continues normally. SQLite remains authoritative, and Markdown notes can be regenerated from it.
...
Headings come directly from requests. Model, tool, usage, and cost details come from Pi's records. Ephemeral sessions have no file link, so their session and entry IDs are shown instead.
...
The extension registers one agent tool, `journal_search`. It provides ranked full-text search over recorded requests, with optional filters for:
...
Results include the matching request, project path, model and tool facts, and a link to the native Pi session. Search is intentionally scoped to **what you asked Pi**. Response bodies are not copied into the index, so details mentioned only inside a conversation remain available through the linked transcript rather than journal search.

pi-ledger adds no slash commands or custom UI.
